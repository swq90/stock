...
def f2(start_date='20190926', end_date="20191021", period=12, times=9, pct_low=0, pct_high=0.2):
    stocks = []
    # 获取股票代码list
    # if not end_date:
    #     end_date = "20191017"
    ...
    # 过滤出涨幅满足条件的
    stock_list = stock1[(stock1["pct"] < pct_high) & (stock1["pct"] > pct_low)]
    ...
    res["pct"]= res["pct"].map(lambda x:('%.2f')%x)
    print(res)
