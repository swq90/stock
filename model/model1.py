...
def f1(start_date = '20190920',end_date="20191018", period=15, times=12, pct=0.8):
    stocks = []
    # 获取股票代码list
    # if not end_date:
    #     end_date = "20191017"
    ...
    stock_list = stock_list[stock_list["pct"] >= pct][["ts_code"]]
    ...
    for stock in stock_list['ts_code']:
        df = gd.get_daily(ts_code=stock,start_date=start_date,end_date="20191017")
    ...
    pd.DataFrame(stocks).to_csv(end_date+'p'+str(period)+".csv")
    return stocks
