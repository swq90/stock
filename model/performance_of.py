...
def first_up(start_date='20200201', end_date='20200215', PRICEB='open', NEWPRICEB='price_buy', PRICES='close',
             sell_date=1,
             buy_date=2, CUTS=2,
    ...
    if buy_date != 1:
        price_buy = basic.basic().pre_data(
            ...
            new_label=[NEWPRICEB, 'xishu_pct', ],
            pre_days=-buy_date + 1)
        raw_data = raw_data.merge(price_buy[['ts_code', 'trade_date', NEWPRICEB, 'xishu_pct']],
                                  on=['ts_code', 'trade_date'])
    ...
    for cut in range(-12, 11, CUTS):
        if cut < -10:
            buy_data = data[data['xishu_pct'] == -9999]
            print('open down')
        elif cut > 8:
            buy_data = data[data['xishu_pct'] == 9999]
            print('open up')
            ...
            buy_data = data[(data['xishu_pct'] < cut + 2) & (data['xishu_pct'] >= cut)]
            print('%s<=x<%s' % (cut, cut + CUTS))
        if buy_data.empty:
            # res_all.loc['%s~%s' % (cut, cut + CUTS), 'res'] = None
            res_some.loc['%s~%s' % (cut, cut + CUTS), 'res'] = None
            print(buy_data['xishu_pct'].max(), buy_data['xishu_pct'].min(),
    ...
    for year in years:
        for month in range(1, 13):
            ...
            if start > '2020025':
                break
            print(start, end)
            z=first_up(start_date=start, end_date=end, PRICEB='open', PRICES='open', sell_date=2, buy_date=2,
                     CUTS=2, CHANGE=['open', 'pre_close'])
            res[start]=z['res']

    res.to_csv('month-of-first-up-limit.csv')
    # first_up(start_date='20190101', end_date='20200215', PRICEB='open', PRICES='close', sell_date=1,  buy_date=2,
    #          CUTS=2, CHANGE=['open', 'pre_close'])
    # first_up(start_date='20190101', end_date='20191231', PRICEB='open', PRICES='close', sell_date=2,  buy_date=2,
    #          CUTS=2, CHANGE=['open', 'pre_close'])

    # first_up(start_date='20190101', end_date='20191231', PRICEB='open', PRICES='open', sell_date=2, buy_date=2,
    #          CUTS=2, CHANGE=['open', 'pre_close'])
